...
数据持有者会监听区块链事件`Create Task`来获取任务，如果选择加入一个任务，则开启监听另一个计算任务轮次的事件通知`Round Start`。等待计算轮次（Round）的开启。

<<<<<<< HEAD
在任务注册阶段，数据持有者不需要在链上报名参加任务。报名参加这个动作发生在每次收到`Round Start`这个事件之后，也就是说，数据持有者不是参加整个任务，而是参加某一个轮次。这样做的理由是，无论是联邦学习还是联邦统计任务，都可能包含很多个轮次，流程很长，在整个任务的流程中，数据的持有者可能会突然掉线，对于数据持有者是手机端的情况，掉线就更加常见了。如果只在任务注册阶段允许数据持有者加入任务，那么任务运行的过程中，节点只会越来越少。而每个轮次的粒度足够小，让节点在每个轮次开始时选择加入，可以在节点经常会掉线的情况下，每个轮次也能有足够多的节点加入。

这种设计针对少节点、单个节点多数据的企业场景也同样适用。相比每次任务注册时，让节点选择加入任务，这种设计当然开销会更大一点。不过加入任务本身这个操作，本身的开销就很小，即使每轮都执行一次，也不会带来很大的负担。
=======
...
>>>>>>> 0883f6a96f02d2935a3419b413870878d62f8d3b
...
一个隐私计算任务，无论是联邦学习还是联邦统计，任务执行的基本单位都是轮次（Round）。

在上一节，我们已经提到了，在每个轮次开始时，数据持有者都可以选择加入这个轮次，这时候，这些加入的节点被称为候选节点。但是，并不是所有的候选节点都可以加入这个轮次的。任务发起者，需要对加入的候选节点进行筛选，选出真正能加入这个轮次的节点，即参与节点。这就时参与者选取。

我们需要对参与者进行筛选，主要是因为加入的节点并不是越多越好的。加入的节点越多，在后续安全聚合的过程中，数据传输、同步的开销也就越大，所以节点数量需要有个上限。任务发起者可以根据需要，在创建任务时对这个上限进行设置。

同样，参与者选取的策略，也是一个可以设置的项。通常来说，对于联邦学习任务，参与者选取的策略通常是随机选取，而联邦统计任务，参与者选取的策略则是选取与上一轮相同的节点。

对于联邦学习任务来说，任务是训练一个机器学习模型。在每一个轮次中，任务的训练需要随机选择一批样本，用来对模型进行更新。为了保证随机性，除了每个节点自身会在本地数据集上进行随机选取外，我们还需要对节点也进行随机选取，这样才能防止因为节点选取造成的数据偏斜。

而对于联邦统计任务来说，任务是在数据上进行统计。每个轮次的计算之间，都可能会有依赖关系，如果在轮次之间选择的节点不同，数据也不同，那么最后统计出的结果就失去意义了。例如，如果我们要同时计算数据集的均值与方差，均值与方差会在两个轮次中分别计算。如果这两个轮次的选取的数据不同，那么均值与方差之间就失去了关联，没有意义了。所以在联邦统计任务中，选取的策略是选取与上一轮相同的节点。当然，第一个轮次中，节点的选取还是随机的。
...
至此一个轮次就完成了，服务器端拿到了聚合后的计算结果，计算节点未对外暴露任何数据，同时区块链上有足够的数据对这次计算进行验证和结算。
